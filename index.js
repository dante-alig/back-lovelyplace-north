...
app.post("/place/hours", async (req, res) => {
  try {
    ...
    if (placesResponse.data.result && placesResponse.data.result.opening_hours) {
      return res.json({
    ...
    return res.status(500).json({
      error: error.response?.data?.error_message || "Erreur lors de la récupération des horaires"
    });
  }
});
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: 799, // 20€ en centimes
          },
          quantity: 1,
        },
      ],
